...
Original file is located at
    https://colab.research.google.com/drive/1CvZW95xq7qGzjbndG_o1AcIc_UV5baTz
"""
...
frame = cv2.imread('/content/images.jpeg')  # Use this if no camera
cv2_imshow(frame)
...
cv2.imwrite('/content/images.jpeg', equalized)    # Save for next
...
blur = cv2.GaussianBlur(gray, (5,5), 0)       # Denoise
equalized = cv2.equalizeHist(blur)            # Contrast enhancement
cv2_imshow(equalized) # Use cv2_imshow instead of cv2.imshow
...
# cv2.waitKey(0)
# cv2.destroyAllWindows()
cv2.imwrite('segmented.jpg', output) # Uncommented this line to save the segmented image
...
img2 = cv2.imread('/content/VK.jpeg', 0)  # Assume a second image
orb = cv2.ORB_create()
